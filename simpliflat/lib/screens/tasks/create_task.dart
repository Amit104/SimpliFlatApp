...
class CreateTask extends StatefulWidget {
  ...
  CreateTask(this.taskId, this._flatId, this.typeOfTask);

  @override
  State<StatefulWidget> createState() {
    return _CreateTask(taskId, _flatId, typeOfTask);
  }
}
...
class _CreateTask extends State<CreateTask> {
  ...
  Map<int, String> repeatMsgs = {-1: 'Occurs Once', 0:'Set Daily',1:'Always Available',2:'Set weekly',3:'Set weekly on particular days',4:'Set Monthly',5:'Set monthly on particular dates'};
  ...
  _CreateTask(this.taskId, this._flatId, this.typeOfTask) {
                ...
                child: taskId == null
                    ...
                    : StreamBuilder(
                        ...
                        builder: (context, snapshot) {
                            ...
                            if (taskId != null && !initialized) { /** if edit task */
                              /** process data receieved from database and assign */
                              debugPrint(snapshot.data['title'] + ' ' + snapshot.data['priority'].toString() + ' ' +snapshot.data['duration'].toString() + ' ' +
                              snapshot.data['repeat'].toString() + ' ' + snapshot.data['frequency'].toString() + ' ' + snapshot.data['assignee'].toString() + ' ' + snapshot.data['due'].toString() + ' ' +
                              snapshot.data['notes']);
                              /** get task name */
                              ...
                              if(snapshot.data['duration'] != '') {
                                var durationElements = snapshot.data['duration'].split(":");
                                _duration = new Duration(hours: int.parse(durationElements[0]), minutes: int.parse(durationElements[1]));
                              ...
                              if(snapshot.data['frequency'] != '')
                                _selectedFrequencies = snapshot.data['frequency'].toString().split(',').map(int.parse).toSet();
                              else
                              ...
                              if(_selectedUser != '')
                                selectedUsers.addAll(_selectedUser.split(',').toList());
                              ...
                              duebefore = (snapshot.data['due'] as Timestamp).toDate();
                              _selectedTime = new TimeOfDay(hour: _selectedDate.hour, minute: _selectedDate.minute);
                              ...
                              /** get remindIssue */
                              _isRemindMeOfIssueSelected = snapshot.data['remindIssue'];
                              /** get remindIssue ends */

                              /** get payment amount */
                              paymentAmountController.text = snapshot.data['paymentAmount'].toString();
                              ...
                              if(snapshot.data['nextDueDate'] != null)
                                _nextDueDate = (snapshot.data['nextDueDate'] as Timestamp).toDate();
                              else
                                _nextDueDate = (snapshot.data['due'] as Timestamp).toDate();;
                              /** get next due date ends */

                              _remind = snapshot.data['shouldRemindDaily'] ?? false;
                              
                              initialized = true;
                              debugPrint(tc.text + ' ' + _selectedDate.toIso8601String() + ' ' + _selectedTime.toString() + ' ' + _selectedPriority
                                 + ' ' + selectedUsers.toList().join(",") + ' ' + _repeat.toString() + 
                                ' ' + _selectedFrequencies.join(",") + ' ' + _durationStr + ' ' + notescontroller.text);
                              //return buildForm(snapshot.data);
                              /** data processed and assigned */
                            } else if (taskId == null) {
                              tc.text = ""; notescontroller.text = "";
                            }
  ...
  Widget _getSaveButtonWidget() {
    return Expanded(
                          ...
                          if (errorMsg == '') {
                            ...
                            DateTime temp = new DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day, _selectedTime.hour, _selectedTime.minute);
                              ...
                              double paymentAmount = (typeOfTask == 'Payment')? double.parse(paymentAmountController.text): 0.0;

                                  debugPrint(task + ' ' + duedatetime.toString() + ' ' + _selectedPriority
                               + ' ' + selectedUsers.toList().join(",") + ' ' + _repeat.toString() + 
                              ' ' + _frequencies + ' ' + _durationStr + ' ' + notestext);
                            ...
                            if (taskId == null) {
                              ...
                              _nextNewDueDate =  Timestamp.fromDate(getNextDueDateTime(DateTime.now(), duedatetime.toDate(), _repeat, _frequencies));
                              ...
                              if(duebefore.compareTo(duedatetime.toDate()) != 0 || repeatbefore != _repeat) { //add repeat not changed condition too
  ...
  Widget _getDueDateTimeWidget(String nextDueDate) {
    debugPrint('new datetime = ' + nextDueDate);
          ...
          return Container(
            decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(5.0),border: Border.all(width: 1.0, color: Colors.grey[300])),
            child: ListTile(
              dense: true,
                leading: Column(
    ...
    children: <Widget>[
      Icon(Icons.calendar_today, color: Colors.blueGrey,),
                ...
                title: Text('Due', style: const TextStyle(
                        color: Colors.black,
                        fontSize: 16.0,
                        fontFamily: 'Montserrat',
                        )),
                
                subtitle: nextDueDate == ''? null: Text(nextDueDate, style: const TextStyle(
                        fontSize: 13.0,
                        fontFamily: 'Montserrat',
                        )),
                
                trailing: Row(mainAxisAlignment: MainAxisAlignment.end, mainAxisSize: MainAxisSize.min, children:[
                          Visibility(visible: showConflictsWarningSign, child:IconButton(icon: Icon(Icons.warning, color: Colors.red,), onPressed: () {setState(() {
  ...
  Container _getRepeatLayout() {
    ...
    return Container(
      padding: EdgeInsets.only(top: 5.0, bottom: 5.0),
                ...
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(5.0),border: Border.all(width: 1.0, color: Colors.grey[300])),
      
      child:  
      ListTile (
        dense:true,
      leading: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.replay)]),
      title: 
       Text((subtitle == null? '-': subtitle == 3? getWeeksFrequencyMsg(frequencies.toList()): repeatMsgs[subtitle]),
      style: TextStyle(
        color: Colors.black,
        fontSize: 14.0,
        fontFamily: 'Montserrat'),
                            ...
                            builder: (_) => new Form(
                                  ...
                                  child: AlertDialog(
                                    ...
                                    content: new SingleChildScrollView(
                                      child: new Material(
                                        child: new RepeatDialog(this.getFrequency, _repeat, _selectedFrequencies),
                                      ),
                                    ),
                                  ),
                                ),
  ...
  Widget _getPaymentInfoWidget() {
    ...
    return Container(
      child: Column(
        ...
        children: [
          ListTile(
            dense:true,
            trailing: Icon(Icons.payment, color: Colors.lightBlue[400],),
    ...
    children: <Widget>[
      Icon(Icons.person, color: Colors.blueGrey,),
                ...
                title: Text('Payee', style: const TextStyle(
                        color: Colors.black,
                        fontSize: 16.0,
                        fontFamily: 'Montserrat',
                        )),
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ListTile(dense: true,
                  ...
                  title: Text('NOTES', style: const TextStyle(
                          color: Colors.black,
                          fontSize: 14.0,
                          fontFamily: 'Montserrat',
                          )),
                ),
        Container(alignment:Alignment.center,margin:EdgeInsets.only(left:10.0, right:5.0, bottom:4.0), child:TextFormField (
                maxLines: null,
  ...
  Widget _getDurationWidget() {
    return ListTile(
                          title: _durationStr == ''? Text('Duration',
                          style: TextStyle(
                            color: _durationStr == ''? Colors.grey[500]: Colors.black,
                          ),
                          ): Text(getFormattedDurationString()),
                          leading: Icon(Icons.timer, color: Colors.indigo[400],),
                          onTap: () {
                            showDialog(
                              context: context,
                              builder: (_) => new Form(
                                    ...
                                    child: AlertDialog(
                                      ...
                                      content: new SingleChildScrollView(
                                        child: new Column (
                                          children: [CupertinoTimerPicker(
                                            initialTimerDuration: _duration == null? new Duration(): _duration,
    ...
    return Visibility (
                        ...
                        child: Container (
                          ...
                          child: Card (
                            child: Column (
                              children: [
                                Container(color:Colors.grey[400], child:ListTile(title: Text('Conflicts'), trailing: IconButton(icon:Icon(Icons.close), onPressed: () {setState(() {
                                                              openConfictsView = false;
                                                            });},))),
                                Scrollbar(
                                              
                                                                  child: ConstrainedBox(constraints: BoxConstraints(maxHeight: 100.0), child:ListView.separated(
                                    ...
                                    itemBuilder: (BuildContext context, int index) {
                                      ...
                                      if(repeatTemp == 3 || repeatTemp == 5) {
                                        freqTemp  = (tasksWithConflicts[index]['frequency'] as String).split(',').map(int.parse).toList();
                                      }
                                      return ListTile(title: Text(tasksWithConflicts[index]['title']),
                                      subtitle: Text(repeatTemp == 3? getWeeksFrequencyMsg(freqTemp.toList()): repeatMsgs[repeatTemp]),
                                      );                   
                                    },
                                  )),
                                ),
      ...
      child: Form (
        ...
        child: Column (
          ...
          children: <Widget>[
            ...
            typeOfTask != 'Complaint'?
            _getRepeatLayout():Container(),

            typeOfTask == 'Reminder'?
            SizedBox(height: 20.0):Container(),

            typeOfTask == 'Reminder'?
            _getDurationWidget():Container(),
            ...
            typeOfTask == 'Payment'?
            _getPaymentInfoWidget():Container(),
  ...
  DateTime getNextDueDateTime(DateTime nowDueDate, DateTime due, int repeat, String frequency) {
    ...
    switch(repeat) {
      ...
      case 2: {
        ...
        return new DateTime(now.year, now.month, now.day, due.hour, due.minute).add(new Duration(days: 7));
      ...
      case 3: {
        ...
        for(int i = 0; i < taskFreq.length; i++) {
          if((taskFreq[i] == now.weekday && nowTime < dueTime) || taskFreq[i] > nowDueDate.weekday) {
            taskDay = taskFreq[i];
            break;
          }
      ...
      case 4: {
        ...
        return new DateTime(year, month, nowDueDate.day, due.hour, due.minute);
      ...
      case 5: {
        ...
        int taskDay = -1;
        for(int i = 0; i < taskFreq.length; i++) {
          if((taskFreq[i] == now.day && nowTime < dueTime) || taskFreq[i] > nowDueDate.day) {
            taskDay = taskFreq[i];
            break;
          }
        ...
        return new DateTime(year, month, taskDay, nowDueDate.hour, nowDueDate.minute);
  ...
  getFrequency(Set<int> frequencies, int repeat) {
    ...
    setState(() {
      if(repeat == 1) {
        ...
        _selectedTime = TimeOfDay(hour: _selectedDate.hour, minute: _selectedDate.minute + 1);
      }
      else {
      _selectedDate = null;
      _selectedTime = null;
  ...
  Future<Null> _selectDate(BuildContext context) async {
    ...
    if(_repeat == 1) {
      showDialog(context: context, barrierDismissible: true, builder: (context) {
        return AlertDialog(content: Text('Not allowed to set due date time when repeat is set to always available'));
      });
      return;
    }
    if(_repeat == -1) {
    ...
    if ((timePicked != null &&
        timePicked != _selectedTime)) {
            String freq = _selectedFrequencies == null? '' : _selectedFrequencies.join(',');
            DateTime now = DateTime.now();

            picked = getNextDueDateTime(now, new DateTime(now.year, now.month, now.day, timePicked.hour, timePicked.minute), _repeat, freq);
  ...
  ///get all tasks to check conflicts
   Future<List<DocumentSnapshot>> _getAllTasks() async {
    QuerySnapshot tasks = await Firestore.instance
                          .collection(globals.flat)
                          .document(_flatId)
                          .collection(globals.tasks).where("completed", isEqualTo: false).getDocuments();
  ...
  void _getTasksWithConflicts() {
    ...
    _getAllTasks().then((tasksList) {
      if(tasksList == null)
        return;
      DateTime duedatetime = new DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day, _selectedTime.hour, _selectedTime.minute);
      ...
      for(int i = 0; i < tasksList.length; i++) {
          ...
          if(tasksList[i].documentID == taskId) {
            continue;
                ...
                debugPrint(tasksList[i].data['title']);
                                debugPrint((tasksList[i].data['due']as Timestamp).toDate().toIso8601String());
        ...
        DateTime existingduedatetime = (tasksList[i].data['due'] as Timestamp).toDate();
        DateTime existingtoduedatetime = existingduedatetime.add(new Duration(hours: 0, minutes: 1));
        ...
        if(tasksList[i].data['duration'] != '') {
          ...
          existingtoduedatetime = existingduedatetime.add(new Duration(hours: hours, minutes: minutes));
        ...
        if(taskRepeat == 3 && _repeat == 2) {
          List<int> frequency = tasksList[i].data['frequency'].split(',').map(int.parse).toList();
          if(frequency.contains(duedatetime.weekday) && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
          ...
          if(frequency.contains(existingduedatetime.weekday)  && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          }
        }
          ...
          List<int> frequency = tasksList[i].data['frequency'].split(',').map(int.parse).toList();
          if(frequency.contains(duedatetime.day)  && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          }
        }
          ...
          if(frequency.contains(existingduedatetime.day)  && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          }
        }
        else if(taskRepeat == 2 && _repeat == 2) {
          if(duedatetime.weekday == existingduedatetime.weekday &&
            _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
              tasksWithConflicts.add(tasksList[i].data);
          }
        }
          ...
          Set<int> frequency1 = (tasksList[i].data['frequency'] as String).split(',').map(int.parse).toList().toSet();
          
          if(frequency1.intersection(_selectedFrequencies).isNotEmpty && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          ...
          Set<int> frequency1 = tasksList[i].data['frequency'].split(',').map(int.parse).toList().toSet();
          if(frequency1.intersection(_selectedFrequencies).isNotEmpty && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          } 
        }
          ...
          if(duedatetime.weekday == existingduedatetime.weekday && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          } 
        }
        else if(taskRepeat == -1 && _repeat == 3) {
          if(_selectedFrequencies.contains(existingduedatetime.weekday) && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          } 

        }
        else if(taskRepeat == -1 && _repeat == 4) {
          if(duedatetime.day == existingduedatetime.day && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          } 
          ...
          if(_selectedFrequencies.contains(existingduedatetime.day) && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          } 
          ...
          if(frequency1.contains(duedatetime.weekday) && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          } 
          ...
          if(frequency1.contains(duedatetime.day) && _overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
            tasksWithConflicts.add(tasksList[i].data);
          } 

        }
        
        else if(_overlap(duedatetime, toduedatetime, existingduedatetime, existingtoduedatetime)) {
          debugPrint(tasksList[i].data['title']);
          tasksWithConflicts.add(tasksList[i].data);
      ...
      for(int i = 0; i < tasksWithConflicts.length; i++) {
        debugPrint(tasksWithConflicts[i]['title']);
      }
  ...
  bool _overlap(DateTime newTaskFrom, DateTime newTaskTo, DateTime existingTaskFrom, DateTime existingTaskTo) {
    existingTaskFrom = new DateTime(newTaskFrom.year, newTaskFrom.month, newTaskFrom.day, existingTaskFrom.hour, existingTaskFrom.minute);
    existingTaskTo = new DateTime(newTaskTo.year, newTaskTo.month, newTaskTo.day, existingTaskTo.hour, existingTaskTo.minute);
    debugPrint(newTaskFrom.toIso8601String() + ' -- ' + newTaskTo.toIso8601String() + ' -- ' + existingTaskFrom.toIso8601String() + ' -- ' + existingTaskTo.toIso8601String());
    return !(newTaskFrom.isAfter(existingTaskTo) || existingTaskFrom.isAfter(newTaskTo));
  }
...
//            ),
            (_selectedFreq == 'Daily')
                ...
                : (_selectedFreq == 'Weekly')
                    ...
                    :(_selectedFreq == 'Monthly')? _showMonthlyWidget(context): Container(),
            Padding(
                padding: EdgeInsets.only(top: 5.0, bottom: 5.0),
                child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      ...
                      OutlineButton(
                          ...
                          onPressed: () {
                            ...
                            showDialog(context: context, barrierDismissible: false, builder: (context) {
                              return AlertDialog(content: Text('Changing repeat will reset due date and time'), actions: <Widget>[
                                OutlineButton(shape: new RoundedRectangleBorder(
                            borderRadius: new BorderRadius.circular(10.0),
                            side: BorderSide(
                              width: 1.0,
                              color: Colors.indigo[900],
                    ...
                    children: List.generate(31, (position) {
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: CircleAvatar(
                            backgroundColor: _selectedDates
                                    .contains((position + 1))
                                ? Colors.indigo[300]
                                : Colors.indigo[100],
                            child: Text(
